...
if [[ "$has_evaluation" == "1" ]]; then
  ...
  tasks/circle_packing/launchers/slurm_attached/stage45.sh
  exit 0
fi

if [[ "$has_generation" == "1" ]]; then
  # Generation exists but evaluation is missing.
  tasks/circle_packing/launchers/slurm_attached/stage3.sh
  tasks/circle_packing/launchers/slurm_attached/stage45.sh
  exit 0
fi

# Fresh epoch: run all three stage bundles.
tasks/circle_packing/launchers/slurm_attached/stage12.sh
tasks/circle_packing/launchers/slurm_attached/stage3.sh
tasks/circle_packing/launchers/slurm_attached/stage45.sh
