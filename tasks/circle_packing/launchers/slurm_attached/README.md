...
Note: each circle packing rollout uses 1 CPU core and can evaluate for up to ~530 seconds, and a full epoch has 512 rollouts. Make sure your CPU array partition has enough cores to keep evaluation time reasonable.
...
bash tasks/circle_packing/launchers/slurm_attached/run_one_epoch.sh
```
...
| `SLURM_ATTACHED_CPUS_PER_EVAL` | CPUs per eval shard (1 for circle packing) |
