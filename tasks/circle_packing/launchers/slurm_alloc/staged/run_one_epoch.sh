...
: "${SLURM_ALLOC_STAGED_JOB_PREFIX:=nanodisc-circle_packing-e$(printf "%03d" "$CHAIN_EPOCH")}"
...
if [[ "$has_generation" != "1" ]]; then
  ...
  job12=$(sbatch --parsable \
    ...
    tasks/circle_packing/launchers/slurm_alloc/staged/stage12.sbatch.sh)
  upstream_dep="$job12"
fi
...
if [[ "$has_evaluation" != "1" ]]; then
  ...
  job3=$(sbatch --parsable \
    ...
    --error="$SLURM_ALLOC_STAGED_STDERR_PATH" \
    --export=ALL,NANODISCOVER_CHAIN_EPOCH="$CHAIN_EPOCH" \
    tasks/circle_packing/launchers/slurm_alloc/staged/stage3_array.sbatch.sh)
  ...
  job3m=$(sbatch --parsable \
    ...
    --error="$SLURM_ALLOC_STAGED_STDERR_PATH" \
    --export=ALL,NANODISCOVER_CHAIN_EPOCH="$CHAIN_EPOCH" \
    tasks/circle_packing/launchers/slurm_alloc/staged/stage3_merge.sbatch.sh)
  upstream_dep="$job3m"
fi
...
job45=$(sbatch --parsable \
  ...
  --error="$SLURM_ALLOC_STAGED_STDERR_PATH" \
  --export=ALL,NANODISCOVER_CHAIN_EPOCH="$CHAIN_EPOCH" \
  tasks/circle_packing/launchers/slurm_alloc/staged/stage45.sbatch.sh)
