...
This folder contains launchers for running `nanodiscover` on the AC2 (2nd autocorrelation inequality) task.
...
export NANODISCOVER_AC2_CONFIG=qwen3_8b_4xL40S
export NANODISCOVER_EVAL_PYTHON=/path/to/nanodiscover-eval-ac2-env/bin/python
...
Logs and artifacts are written under `NANODISCOVER_LOG_ROOT` for fresh runs, or `NANODISCOVER_RESUME_DIR` for resumed runs. On a cluster, symlink `logs/` to networked storage or scratch, as AC2 runs can produce large artifacts.
