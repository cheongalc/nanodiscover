...
Note: each AC2 rollout needs 2 CPU cores and can evaluate for up to ~18 minutes, and a full epoch has 512 rollouts. Make sure your CPU array partition has enough cores to keep evaluation time reasonable.
...
export NANODISCOVER_EVAL_PYTHON=/path/to/nanodiscover-eval-ac2-venv/bin/python
...
bash tasks/ac2/launchers/slurm_attached/run_one_epoch.sh
```
...
| `SLURM_ATTACHED_CPUS_PER_EVAL` | CPUs per eval shard (2 for AC2) |
