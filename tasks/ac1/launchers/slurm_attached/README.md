...
Generation and training run directly on the GPU node you are on. For evaluation, the script splits work between the node's local CPUs and remote Slurm CPU array jobs, so all 512 rollouts can be evaluated in parallel. For example, on a node with 64 CPU cores: 32 rollouts evaluate locally (2 cores each), and the remaining 480 are dispatched to the CPU array partition. If local evaluation finishes first and remote jobs are still queued, the script will pull remaining work back to local CPUs rather than waiting. This is the mode that has been most tested.
...
bash tasks/ac1/launchers/slurm_attached/run_one_epoch.sh
```
...
| `SLURM_ATTACHED_CPUS_PER_EVAL` | CPUs per eval shard (2 for AC1) |
