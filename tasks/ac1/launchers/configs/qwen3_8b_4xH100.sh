...
export RAY_NUM_CPUS=48  # this controls the number of CPUs Ray can use, if you don't set it, Ray will use all available CPUs
export NANODISCOVER_EVALUATOR_NUM_WORKERS=48  # local CPU eval workers; additional workers come from Slurm array jobs when using slurm_attached
...
export NANODISCOVER_TRAINER_NUM_WORKERS=4  # DeepSpeed processes (one per GPU)
export NANODISCOVER_TRAINER_MAX_TOKENS_PER_RANK=16384  # max packed tokens per GPU AFTER Ulysses SP splitting. Multiply this number by the SP size set below to get the pre-split budget. That budget tells you how many sequences can be packed together before SP splits them across the ranks.
export NANODISCOVER_REFERENCE_SCORING_MAX_TOKENS_PER_RANK=16384  # per-rank token budget for batching rollouts during reference policy KL scoring; each rank scores its own shard of the rollouts
...
export NANODISCOVER_SEQUENCE_PARALLEL_SIZE=4  # Ulysses sequence parallelism degree; for now, this should equal TRAINER_NUM_WORKERS because we don't support tensor parallelism yet
