...
export NANODISCOVER_TRAINER_MAX_TOKENS_PER_RANK=12288  # max packed tokens per GPU AFTER Ulysses SP splitting. Multiply this number by the SP size set below to get the pre-split budget. That budget tells you how many sequences can be packed together before SP splits them across the ranks.
...
export NANODISCOVER_STAGE_STOP=archive_update
